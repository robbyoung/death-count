...
export const getAllDeaths = (state: State) =>
    state.deaths.filter(death => death.complete);

export const getIncompleteDeath = (state: State) =>
    state.deaths.find(death => !death.complete);
...
export const getDeathsForCurrentPlaythrough = createSelector(
    ...
    (deaths, selectedPlaythrough) => {
        return deaths.filter(
            death => death.playthroughId == selectedPlaythrough.id,
        );
    },
);
...
export const getDeathsForCurrentGame = createSelector(
    ...
    (deaths, playthroughs, selectedGame) => {
        const playthroughsForGame = playthroughs.filter(
            playthrough => playthrough.gameId === selectedGame.id,
        );
        return deaths.filter(death =>
            playthroughsForGame.some(
                playthrough => playthrough.id === death.playthroughId,
            ),
        );
    },
);
